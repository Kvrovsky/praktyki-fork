...
<script>
	...
	onMount(async () => {
		try{
		const response = await fetch('http://localhost:8899/api/v1/users');
	...
	<div class="card-div">
		...
		<div class="card">
			...
			<div class="card-stats">
				...
				<div class="stat">
					<div class="value">{formatDistance(new Date(user.created_at), new Date(), { addSuffix: true })}</div>
                    <!-- <span class="date">{formatDistance(new Date(job.created_at), new Date(), { addSuffix: true })} from {job.city}</span> -->
					<div class="type">register date</div>
				</div>
			</div>
		</div>
		{/each}
